<template>
  <div class="media_audit_center" v-if="showCenter">
    <ul class="audit_top">
      <li>
    ...
    <div class="audit_content">
      <el-radio-group v-model="audit_type" @change="fetchAuditMenuListFn">
      ...
      <div v-if="auditIndexList.length">
        <ul class="audit_content_ul">
          <li v-for="item in auditIndexList" :key="item.name">
            ...
            >
              <el-checkbox class="Mr8" label="image">图片</el-checkbox>
              <el-checkbox class="Mr8" label="video">视频</el-checkbox>
              <el-checkbox class="Mr8" label="audio">音频</el-checkbox>
            </el-checkbox-group>
          </li>
        </ul>
      </div>
  ...
  <div class="media_audit_list" v-else>
    ...
    <el-row class="audit_list_main" v-loading="auditLoading">
      <el-col :span="4" v-for="(item, index) in auditList" :key="index">
        <MediaAuditBox
          :ref="`audit${item.id}`"
          :auditData="item"
...
// import { fetchOptsSecondTags } from '@/apis/options'
import MediaAuditBox from '@/components/media_audit_box/MediaAuditBox.vue'

const CONTENT_TYPES_STORE = 'mediaAuditContentTypes'
const DEFAULT_CONTENT_TYPES = ['image', 'video', 'audio']
